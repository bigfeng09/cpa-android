...
public class MainActivity extends Activity {
    ...
    private String currentQuotaUrl() { String value = quotaUrl == null ? "" : quotaUrl.trim(); if (prefs != null) { String saved = prefs.getString(KEY_QUOTA_URL, value.length() == 0 ? DEFAULT_QUOTA_URL : value); if (saved != null && saved.trim().length() > 0) value = saved.trim(); } String corrected = correctedQuotaUrl(value); if (prefs != null && !corrected.equals(value)) prefs.edit().putString(KEY_QUOTA_URL, corrected).apply(); quotaUrl = corrected; return corrected; }
